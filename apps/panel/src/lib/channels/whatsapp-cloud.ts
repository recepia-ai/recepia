...
export type ChannelRow = Database["public"]["Tables"]["clinic_channels"]["Row"];
...
export function configObject(value: ChannelRow["provider_config"]): Record<string, unknown> {
...
export function parseWhatsAppCloudWebhook(payload: unknown): WhatsAppCloudWebhook {
  return whatsAppCloudWebhookSchema.parse(payload);
}

export async function resolveWhatsAppCloudChannel(
  ...
  if (!channel) {
    throw new Error(`No hay un canal ${provider} activo para ${displayPhoneNumber}`);
  }
  return channel;
}

export async function resolveClinicWhatsAppCloudChannel(
  supabaseAdmin: AdminClient,
  clinicId: string,
  provider: string,
): Promise<ChannelRow> {
  const { data, error } = await supabaseAdmin
    ...
    .eq("status", "active")
    .maybeSingle();
  if (error || !data) throw new Error(`No hay un canal ${provider} activo para esta clínica`);
  return data;
}

export function inboundEventsFromWhatsAppCloud(
  webhook: WhatsAppCloudWebhook,
  channel: ChannelRow,
  provider: string,
): InboundChannelEvent[] {
...
export async function readWhatsAppCredential(
  ...
  if (error || !data) throw new Error(`No se pudo leer la credencial de ${channel.provider}`);

  try {
    const parsed = JSON.parse(data as string) as { access_token?: string; api_key?: string };
    const credential = parsed.access_token ?? parsed.api_key;
    if (credential) return credential;
  } catch {
    // Raw credentials remain supported for channels created before the structured Vault format.
  }
  return data as string;
}

export async function applyWhatsAppCloudStatuses(
  supabaseAdmin: AdminClient,
  webhook: WhatsAppCloudWebhook,
  provider: string,
): Promise<void> {
    ...
    for (const change of entry.changes) {
      for (const status of change.value.statuses ?? []) {
        const { data: message } = await supabaseAdmin
          .from("messages")
          .select("id, metadata")
          .eq("provider_message_id", `${provider}:${status.id}`)
          .maybeSingle();
        if (!message) continue;
